...
void installObject(QObject *fromObj, const QString &toName, QJSEngine *engine)
{
    PerformanceLogger logger( QString("installObject::%1").arg(toName) );

    static int i = 0;
    const auto fromName = QString("_copyq%1").arg(++i);
    const auto from = engine->newQObject(fromObj);
    engine->globalObject().setProperty(fromName, from);

    const auto metaObject = fromObj->metaObject();

    QJSValueIterator it(from);
    QString script;
    while (it.hasNext()) {
        it.next();
        const auto name = it.name();
        if ( name.startsWith(QLatin1String("objectName")) )
            continue;

        const auto nameBytes = name.toUtf8();
        const int i = metaObject->indexOfProperty(nameBytes.data());
        if (i == -1) {
            const auto slotName = nameBytes + "()";
            const int j = metaObject->indexOfSlot(slotName.data());
            if (j == -1)
                continue;
...
{
    static const QString script = "_copyqArguments.length";
    return evaluateStrict(m_engine, script).toInt();
}

QJSValue Scriptable::argument(int index) const
{
    static const QString script = "_copyqArguments";
    return evaluateStrict(m_engine, script).property( static_cast<quint32>(index) );
}
...
QJSValue Scriptable::throwError(const QString &errorMessage)
{
    ...
    engine()->globalObject().setProperty("_copyqExceptionText", errorMessage);
    static const QString script = "throw new Error(_copyqExceptionText)";
    setUncaughtException( engine()->evaluate(script, script) );
...
QJSValue Scriptable::getPlugins()
{
    ...
    if ( m_plugins.isUndefined() ) {
        ...
        for (auto obj : scriptableObjects) {
            ...
            m_plugins.setProperty(name, engine()->newObject());
            installObject(obj, QString("_copyqPlugins.%1").arg(name), m_engine);
...
bool Scriptable::sourceScriptCommands()
{
    ...
    for (const auto &command : commands) {
        const auto script = QString("(function(){\n%1\n;})()").arg(command.cmd);
        eval(script, command.name);
        if ( hasUncaughtException() ) {
...
int Scriptable::executeArguments(const QStringList &args)
{
    ...
    while ( skipArguments < fnArgs.size() && canContinue() && !hasUncaughtException() ) {
        if ( result.isCallable() ) {
            ...
            const QString label = QString("eval(arguments[%1])").arg(skipArguments);
...
QString Scriptable::processUncaughtException(const QString &cmd)
{
    ...
    if (label.isEmpty()) {
        logScriptError(
            QString("Exception in command: %1")
            .arg(exceptionText) );
    } else {
        logScriptError(
...
QJSValue Scriptable::eval(const QString &script)
{
    ...
    const QString name = QString("{{ %1 }}").arg( i == -1 ? script : script.mid(0, i) + "..." );
